...
void zk_assert_linear(const IntFp *c0W_fp, const IntFp *v_fp,
                      const IntFp &c0u_fp, const uint64_t a,
...
// ── ZKP 4: ||r[i,T]||^2 <= a^(2t) ──────────────────────────────────────────
void zk_assert_norm_bound(IntFp **r_fp,           // all r[i,j], shape [K*T][F]
                           const uint64_t **r_plain, // plaintext r[i,j], shape [K*(T+1)][F]
                           const uint64_t a_val, const int F,
                           const char* label) {
    ...
    for (int i = 0; i < PARAM_K; ++i) {
        ...
        for (int f = 0; f < F; ++f)
            norm_sq_plain = (norm_sq_plain + (__uint128_t)r_iT_plain[f] * r_iT_plain[f]) % pr;
...
void test_nn_fair(BoolIO<NetIO> *ios[THREADS], int party, int f) {
    ...
    for (int i = 0; i < PARAM_K * PARAM_T; ++i) {
        ...
        for (int k = 0; k < F; ++k) lambdas[i][k] %= pr;
    }
    for (int i = 0; i < F; ++i) { lambda_u[i] %= pr; lambda_v[i] %= pr; }

    IntFp *lambda_u_fp = new IntFp[F];
    IntFp *lambda_v_fp = new IntFp[F];
    IntFp **lambdas_fp = new IntFp*[PARAM_K * PARAM_T];
    for (int i = 0; i < F; ++i) lambda_u_fp[i] = IntFp(lambda_u[i], ALICE);
    for (int i = 0; i < F; ++i) lambda_v_fp[i] = IntFp(lambda_v[i], ALICE);
    for (int i = 0; i < PARAM_K * PARAM_T; ++i) {
    ...
    for (int i = 0; i < MAT_N; ++i) W[i] %= pr;
    for (int i = 0; i < F; ++i) { u[i] %= pr; v[i] %= pr; }
    ...
    for (int i = 0; i < PARAM_K; ++i) {
        ...
        for (int j = 1; j <= PARAM_T; ++j) {
            ...
            for (int row = 0; row < F; ++row)
                for (int col = 0; col < F; ++col)
                    tmp[row] = (tmp[row] + (__uint128_t)W[row*F + col] * prev[col]) % pr;
    ...
    // Wr_fp[i*PARAM_T+j] = W * r[i,j-1]  (j-1 = prev step, 0-indexed → r[i*(T+1)+j])
    ...
    for (int i = 0; i < PARAM_K; ++i) {
        for (int j = 0; j < PARAM_T; ++j) {
            ...
            for (int row = 0; row < F; ++row) {
                Wr_fp[idx][row] = pub(0);
                for (int col = 0; col < F; ++col)
                    Wr_fp[idx][row] = Wr_fp[idx][row] + W_fp[row*F + col] * pub(r_prev[col]);
            }
        }
    }
    ...
    auto timeuse = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
    cout << "F=" << F << "\tTime: " << timeuse << "us\tParty: " << party << endl;
